  ...
  events.forEach(async (event, index) => {
    try {
      if (event.type === "message" && event.message?.type === "text") {
        ...
        // Check for the trigger keywords
        ...
        // Get the part of the message after the keyword
        const aiPrompt = messageText.substring(messageText.indexOf(" ") + 1);
        const characterThreshold = 500;
        ...
        // Note: Further AI processing (second layer) would be implemented here
        // to handle the different `case` statements.
      } else if (event.type === "postback") {
        await handlePostback(event);
      } else if (event.type === "follow") {
        console.log(`[${getTimestamp()}] 👋 User followed the bot: ${event.source?.userId}`);
