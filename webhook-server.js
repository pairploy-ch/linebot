    ...
    if (notificationDates.length > 0) {
        ...
        notificationDates.forEach(date => {
            ...
            batch.set(newNotifRef, {
                ...
                nextAt: notificationTimestamp, // Set nextAt on creation
                status: "Upcoming",
                notified: false,
                userId: lineUserId,
            });
...
/**
 * Handles postback events from LINE, including task completion and snoozing.
...
async function handlePostback(event) {
  ...
  if (data.startsWith("complete_task_")) {
    ...
    try {
      ...
      console.log(`[${getTimestamp()}] ✅ Notification marked as Completed: ${notificationId}`);
      await sendReplyMessage(replyToken, [{
        type: "text",
        text: `เยี่ยมมาก! งานเสร็จสิ้นแล้ว 👍`
      }]);
  ...
  // ADDED: Handler for snooze postback
  else if (data.startsWith("snooze_")) {
    const parts = data.split('_');
    // snooze_user_<uid>_task_<parentTaskId>_notification_<notifId>_m_10
    if (parts.length < 10) {
    ...
    if (isNaN(snoozeMinutes)) {
        console.error(`[${getTimestamp()}] ❌ Invalid snooze minutes: ${parts[9]}`);
        await sendReplyMessage(replyToken, [{ type: "text", text: "❌ เกิดข้อผิดพลาดในการเลื่อนเวลา" }]);
        return;
    }
    ...
    try {
        ...
        await notificationRef.update({
            nextAt: Timestamp.fromDate(newDue),
            notified: false, // Reset notified flag to allow re-sending
            status: "Snoozed"
        });

        console.log(`[${getTimestamp()}] ⏰ Notification snoozed for ${snoozeMinutes} minutes: ${notificationId}`);
