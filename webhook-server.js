...
// NEW: This is the backend version of the date calculation.
...
// NEW: handleCreateTask now creates the full task and notifications
async function handleCreateTask(event, prompt) {
  ...
  try {
    ...
    } else if (taskDetails.repeat === 'once') {
        endDate = startDate;
    }
    ...
    // NEW: Create the task in a user-specific subcollection
    const userTasksCollectionRef = db.collection("users").doc(event.source.userId).collection("tasks");
    const docRef = await userTasksCollectionRef.add(masterTask);

    const notificationsCollectionRef = db.collection("users").doc(event.source.userId).collection("tasks").doc(docRef.id).collection("notifications");
...
async function handlePostback(event) {
  ...
  if (data.startsWith("complete_task_")) {
    ...
    try {
      const parentTaskDocRef = db.collection("users").doc(userId).collection("tasks").doc(taskId); // NEW: Correct path
      const notificationsQuery = await parentTaskDocRef.collection("notifications").get();
      ...
      console.log(`[${getTimestamp()}] ✅ Task "${(await parentTaskDocRef.get()).data().title}" marked as Completed`);
      await sendReplyMessage(event.replyToken, [{ type: "text", text: `✅ งาน "${(await parentTaskDocRef.get()).data().title}" ถูกทำเครื่องหมายว่าเสร็จสิ้นแล้ว` }]);
