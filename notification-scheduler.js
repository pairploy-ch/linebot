...
/**
 * Creates a LINE Flex Message for a task notification.
 * Includes "View Task", "Done", and a new "Snooze 10 min" button.
 * @param {object} task - The combined task and notification data.
 * @returns {object} The LINE Flex Message object.
 */
function createTaskFlexMessage(task) {
  const messageDate = moment(task.notificationTime.toDate()).tz('Asia/Bangkok');
  const dateDisplay = messageDate.isValid()
  ...
  return {
    ...
    contents: {
      ...
      footer: {
        type: "box",
        layout: "vertical", // Changed to vertical to better fit 3 buttons
        contents: [
          {
            type: "button",
            style: "primary",
          ...
          // Snooze button added here
          {
            ...
            color: "#f59e0b", // Amber color for snooze
            margin: "sm"
          },
          {
            type: "button",
        ...
        spacing: "sm",
        paddingAll: "10px" // Reduced padding to accommodate buttons
      }
    }
  };
}
...
/**
 * Checks for due notifications based on the `nextAt` field.
 * Sends a notification and updates its status.
 */
async function checkNotifications() {
  const now = moment.tz('Asia/Bangkok');
  const fiveMinutesAgo = now.clone().subtract(5, 'minutes');
  ...
  try {
    ...
    // MODIFIED: Query now uses `nextAt` instead of `notificationTime`
    const notificationsQuery = notificationsRef
      .where('notified', '==', false)
      .where('nextAt', '>=', admin.firestore.Timestamp.fromDate(fiveMinutesAgo.toDate()))
      .where('nextAt', '<=', admin.firestore.Timestamp.fromDate(now.toDate()));

    console.log(`[${getTimestamp()}] ⚙️ Query details for minute check:`);
    console.log(`[${getTimestamp()}]  - Collection Group: 'notifications'`);
    console.log(`[${getTimestamp()}]  - Filter 1: notified == false`);
    console.log(`[${getTimestamp()}]  - Filter 2: nextAt >= ${fiveMinutesAgo.toISOString()}`);
    ...
    for (const notificationDoc of notificationsSnapshot.docs) {
      ...
      if (parentTaskDoc.exists) {
        const parentTaskData = parentTaskDoc.data();
        const nextAtMoment = moment(notificationData.nextAt.toDate());
        
        if (nextAtMoment.isSameOrBefore(now)) {
          const flexMessage = createTaskFlexMessage({
            ...parentTaskData,
            ...notificationData,
            id: notificationDoc.id,
          ...
          // MODIFIED: Update logic after sending
          batch.update(notificationDoc.ref, {
            notified: true,
            status: 'Incomplete', // Mark as Incomplete until user acts
            sentAt: admin.firestore.FieldValue.serverTimestamp(),
          });

          // This logic remains the same
          if (parentTaskData.repeatType === 'Never' || nextAtMoment.isSame(moment(parentTaskData.endDate).tz('Asia/Bangkok'), 'day')) {
            batch.update(parentTaskRef, { status: 'Incomplete' });
          }
        }
      }
...
// ------------------------------------------------
// Functions for the daily summary cron job (Unchanged)
// ------------------------------------------------
